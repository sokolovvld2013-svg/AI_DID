...
| Режим | Назначение |
|-------|------------|
| **Проверка закупочной документации** | Загрузка одного файла (DOCX, TXT, PDF до 50 МБ) → отчёт с блоками 🔴 / 🟡. В контекст LLM попадают **разделы загруженного файла**; 223-ФЗ — из знаний модели и системного промпта. |
| **Экспертное мнение** | Свободный вопрос; при наличии загруженного **Положения о закупке** — поиск по нему (Chroma, коллекция `procurement_kb`) + нормы законодательства. |
