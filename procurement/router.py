...
Опора (иерархия, а не равноправие):
- **Положение о закупке** — приоритет по вопросам, которые оно регулирует; только по переданным фрагментам; в тексте ответа ссылка вида [п. … Положения о закупке], а не служебный номер фрагмента.
- **223-ФЗ** и смежные акты — обязательная императивная основа; из знаний; в тексте ответа ссылка вида [ч. … ст. … 223-ФЗ].
Номера [N] в контексте — служебные, в ответ пользователю их не копируй.

Если вопрос затрагивает сроки или этапы закупки (подача заявок, разъяснения документации, заключение договора) — обязательно дополни ответ императивными нормами 223-ФЗ отдельным аспектом: Положение не вправе установить срок, меньший минимального, предусмотренного законом.
...
async def _query_expert(session_id: str, question: str) -> dict:
    ...
    if context:
        user_prompt += (
            "Фрагменты **Положения о закупке** ниже — приоритет по вопросам, которые оно регулирует. "
            "Обязательно дополняй ответ нормами **223-ФЗ** из собственных знаний. "
