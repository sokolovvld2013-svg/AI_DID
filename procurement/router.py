...
Опора и приоритет источников (иерархия, а не равноправие):
1. **Положение о закупке** — первый приоритет по вопросам, которые оно регулирует; только по переданным фрагментам; в тексте ответа ссылка вида [п. … Положения о закупке], а не служебный номер фрагмента.
2. Если в переданных фрагментах Положения нет ответа на вопрос (нет нужного числа, срока, порядка) или вопрос Положением не регулируется — применяй **223-ФЗ** и смежные акты (обязательная императивная основа, из знаний): закрывай пробел нормой закона; в тексте ответа ссылка вида [ч. … ст. … 223-ФЗ].
3. Если вопрос не урегулирован ни Положением, ни 223-ФЗ — применяй иные актуальные нормативные акты (ГК РФ, постановления Правительства РФ, акты ФАС России и др.).
Номера [N] в контексте — служебные, в ответ пользователю их не копируй.

Сроки и этапы закупки (подача заявок, разъяснения документации, заключение договора):
- Даже если во фрагментах Положения нужный срок отсутствует — обязательно укажи императивные минимальные сроки **223-ФЗ** отдельным аспектом: Положение не вправе установить срок, меньший минимального, предусмотренного законом.
- Называть твёрдо известную императивную норму 223-ФЗ (с точной цифрой и статьёй) — это НЕ галлюцинация: антигаллюцинационные правила запрещают выдумывать несуществующие нормы и «типовые» значения из общих соображений, но не запрещают применять уверенно известную норму закона.

Если фрагменты Положения не переданы — отвечай только по законодательству, без пунктов Положения.

Правила:
- При наличии фрагментов сочетай закон и Положение.
- Если во фрагменте есть прямой ответ (число, срок, процедура) — опирайся на него и укажи пункт Положения, если он есть в тексте фрагмента.
- Не выдумывай пункты Положения и статьи закона.
- Не дроби даты и номера статей (3.4, 18.07.2011) переносом строки.
...
def _build_policy_context(
    ...
    *,
    ...
    hits = _select_relevant_hits(question, hits)
    if not hits:
        return "", []
...
async def _query_expert(session_id: str, question: str) -> dict:
    ...
    if context:
        user_prompt += (
            "Фрагменты **Положения о закупке** ниже — первый приоритет по вопросам, которые оно регулирует. "
            "Если во фрагментах нет прямого ответа (не хватает числа, срока, порядка) — дополняй ответ нормами "
            "**223-ФЗ** из собственных знаний, а при их отсутствии — иными актуальными нормативными актами. "
            "Если вопрос затрагивает сроки и этапы закупки (подача заявок, разъяснения, заключение договора) — "
            "добавь отдельный аспект с императивной нормой 223-ФЗ: Положение не вправе сокращать минимальные сроки закона. "
            "Служебные номера фрагментов [N] в ответ не копируй: в тексте аргументов ставь только числовые "
            "ссылки [1], [2], … — номера из блока «Источники», без дублирования наименования акта и пунктов. "
    ...
    try:
        ...
        answer = ensure_expert_sources_block(
            answer,
            fallback_lines=["Федеральный закон от 18.07.2011 № 223-ФЗ"],
        )
        procurement_history.add(
            session_id,
            question,
