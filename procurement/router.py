...
Опора (равноправны, если переданы фрагменты Положения):
- **223-ФЗ** и смежные акты — из знаний; в тексте ответа ссылка вида [ч. … ст. … 223-ФЗ].
- **Положение о закупке** — только по переданным фрагментам; в тексте ссылка вида [п. … Положения о закупке], а не служебный номер фрагмента.
...
async def _query_expert(session_id: str, question: str) -> dict:
    ...
    if context:
        user_prompt += (
            "Опирайся **равноправно** на **223-ФЗ** и на фрагменты **Положения о закупке** ниже. "
