...
const App = {
    ...
    _boldConclusionHeadings(text) {
        const lead =
            ...
            'Документация|Вердикт|Предмет|НМЦД|Начальная цена|Где|Суть|Обоснование|Источники|Суть вопроса)';
    ...
    _isListSectionBreak(trimmed) {
        ...
        if (
            /^(?:\*\*)?(?:Итоговый|Итого(?![а-яёА-ЯЁ])|Вывод|Заключение|Резюме|Общий вывод|Замечания|Минимальный срок|Суммарно|В итоге|Таким образом|Следовательно|Важно|Обратите внимание|Ответ|Кратко|Документация|Вердикт|Источники|Суть вопроса)(?![А-Яа-яЁё])/i.test(
                trimmed,
            )
        ) {
            return true;
