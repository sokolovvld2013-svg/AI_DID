...
const App = {
    ...
    _normalizeParagraphBreaks(text) {
        const lead =
            ...
            'Суммарно|В итоге|Источники|Суть вопроса)';
        const fields =
    ...
    _isListSectionBreak(trimmed) {
        ...
        if (
            /^(?:\*\*)?(?:Итоговый|Итого(?![а-яёА-ЯЁ])|Вывод|Заключение|Резюме|Общий вывод|Замечания|Минимальный срок|Суммарно|В итоге|Таким образом|Следовательно|Важно|Обратите внимание|Ответ|Кратко|Документация|Вердикт|Источники|Суть вопроса)\b/i.test(
                trimmed,
            )
        ) {
            return true;
