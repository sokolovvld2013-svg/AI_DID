...
const App = {
    ...
    _normalizeListBreaks(text) {
        // Маркер пункта: 1–2 цифры + .| ) + пробел + текст пункта (буква, возможно **жирная**)
        // Не даты 26.07, не п. 17.1 и не «ст. 4.» перед ссылкой [N]
        const listMark = '(?:\\*\\*)?\\d{1,2}[.)](?:\\*\\*)?\\s+(?=(?:\\*\\*)?[A-Za-zА-Яа-яЁё])';
        let s = String(text)
            .replace(/([:;—–])\s*([-*•]\s+\S)/g, '$1\n$2')
            .replace(/(\*\*[^*]+\*\*)\s*([-*•]\s+\S)/g, '$1\n$2')
            .replace(/([^\n])\s+([-*•]\s+\S)/g, '$1\n$2')
    ...
    _normalizeParagraphBreaks(text) {
        const lead =
            ...
            'Суммарно|В итоге|Источники|Суть вопроса)(?![А-Яа-яЁё])';
        const fields =
            '(?<![📄📋])(?:\\*\\*)?(?:Где|Суть|Обоснование|Предмет|НМЦД|Начальная цена|Документация|Договор|Вердикт):';
        return String(text)
            // Не разрывать числовые маркеры списков: «1. Минимальный срок…» нельзя делить
    ...
    _boldConclusionHeadings(text) {
        const lead =
            ...
            'Документация|Договор|Вердикт|Предмет|НМЦД|Начальная цена|Где|Суть|Обоснование|Источники|Суть вопроса)';
    ...
    _repairBrokenDecimals(text) {
        ...
        // «ч. 1 ст.\n4.» → «ч. 1 ст. 4.» (обрыв номера статьи перед ссылкой [N])
        s = s.replace(/([A-Za-zА-Яа-яЁё])\.\s*\n\s*(\d{1,2}\.)/g, '$1. $2');
        return s;
    },
    ...
    _isListSectionBreak(trimmed) {
        ...
        if (/^<(strong|b)>/i.test(trimmed) && /Общий вывод|Замечания|Документация|Договор|Вердикт/i.test(trimmed)) {
            return true;
        }
        // Заголовки итогов/выводов — не продолжение пункта списка
        if (
            /^(?:\*\*)?(?:Итоговый|Итого(?![а-яёА-ЯЁ])|Вывод|Заключение|Резюме|Общий вывод|Замечания|Минимальный срок|Суммарно|В итоге|Таким образом|Следовательно|Важно|Обратите внимание|Ответ|Кратко|Документация|Договор|Вердикт|Источники|Суть вопроса)(?![А-Яа-яЁё])/i.test(
                trimmed,
            )
        ) {
            return true;
    ...
    _isHeadingOnlyLine(trimmed) {
        const head =
            ...
            'Обратите внимание|Ответ|Кратко|Документация|Договор|Вердикт|Предмет|НМЦД|' +
