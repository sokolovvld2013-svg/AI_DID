...
def repair_broken_decimals(text: str) -> str:
    ...
    # «ст.\n2.» → «ст. 2.» (обрыв номера статьи перед источником [N])
    s = re.sub(r"([A-Za-zА-Яа-яЁё])\.\s*\n\s*(\d{1,2}\.)", r"\1. \2", s)
    return s


# Маркер настоящего пункта списка (не дата 26.07, не п. 17.1 и не «ст. 2.» перед [N]):
# 1–2 цифры, точка/скобка, пробел, дальше текст пункта (буква, возможно в **жирном**).
_LIST_MARKER = (
    r"(?:\*\*)?\d{1,2}[.)](?:\*\*)?\s+"
    r"(?=(?:\*\*)?[A-Za-zА-Яа-яЁё])"
)
...
_CONCLUSION_LEAD = (
    ...
    r"Суммарно\b|В итоге\b|Важно\b|Обратите внимание\b|Ответ\b|Кратко\b|"
    r"Документация\b|Вердикт\b|Источники\b|Суть вопроса\b)"
)
...
def ensure_paragraph_breaks(text: str) -> str:
    ...
    # (?<![\d.)]) — не разрывать нумерованные пункты «1. **Заголовок.**»
    s = re.sub(
        rf"(?<![\d.)])([^\n])[ \t]+({_PARAGRAPH_BREAK_LEAD})",
        r"\1\n\2",
        s,
